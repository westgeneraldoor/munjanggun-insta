...
4. `data/registry/INSTAGRAM_POSTING_REGISTRY.md`와 `data/registry/CAROUSEL_SCORECARD_LOG.json`에서 이미 다룬 소재와 점수 이력을 확인합니다.
5. `data/evals/`의 골든셋과 비교해 좋은 예/애매한 예/폐기 예와 같은 패턴인지 확인합니다.
6. `npm run status`에서 제작 가능 후보인지 확인합니다.
7. Hook Score가 7점 이상인지 확인합니다.
8. 기획안을 사용자에게 제시하고 승인을 받습니다.
9. 승인 후 `content/source/carousel/NNN_주제명.md` 원고 파일을 만듭니다.
10. JSON에 `visual_intent`, CTA, caption card, hashtags를 포함합니다.
11. `npm run validate:file -- content/source/carousel/NNN_주제명.md`로 신규 원고를 검증합니다.
12. 원고 생성 작업은 여기서 종료합니다.
