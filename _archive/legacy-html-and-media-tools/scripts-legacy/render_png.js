 ...
 * 
 * 사용법:
 *   node scripts/legacy/render_png.js <html-file> [output-dir]
 *   node scripts/legacy/render_png.js content/published/html-legacy/013_우리집중문선택가이드.html
 *   node scripts/legacy/render_png.js content/published/html-legacy/013_우리집중문선택가이드.html ./output
 * 
...
async function renderSlides(htmlFile, outputDir) {
  ...
  if (!outputDir) {
    ...
    outputDir = path.join(projectRoot, 'outputs', 'exports', path.basename(absolutePath, path.extname(absolutePath)));
  }
...
if (args.length === 0) {
  console.log('사용법: node scripts/legacy/render_png.js <html-file> [output-dir]');
  process.exit(0);
}
