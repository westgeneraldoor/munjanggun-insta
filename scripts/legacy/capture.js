...
async function captureScreenshots() {
  ...
  if (!inputArg) {
    console.error("Usage: node scripts/legacy/capture.js <html-file> [topic]");
    process.exit(1);
  }

  const filePath = path.resolve(projectRoot, inputArg);
  if (!fs.existsSync(filePath)) {
    console.error(`HTML file not found: ${filePath}`);
    process.exit(1);
  }

  const outputDir = path.join(projectRoot, 'outputs', 'previews');
  ...
  const topic = process.argv[3] ? process.argv[3] : path.basename(filePath, path.extname(filePath));
